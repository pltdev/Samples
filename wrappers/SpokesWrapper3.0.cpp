...
string getDeviceState(DeviceChangeState state)
{
	switch(state)
	{
...
extern ICOMATDCommandPtr g_pAtdCommand;
...
public:	
	 ...
	 * Author:		Andrija M. 02/13/2012
	 ****************************************************************/
	STDMETHOD(CallStateEventHandler)(struct ICOMCallEventArgs *args)	
	{
		DeviceEventKind eventKind;
		CallState callState;
		args->get_DeviceEventKind(&eventKind);
		args->get_CallState(&callState);

		CComPtr<ICOMCall> call;
		long callId;
		if( SUCCEEDED (args->get_call(&call) ) )
		{
			call->get_Id(&callId);
	 ...
	 * Author:		Andrija M. 02/13/2012
	 ****************************************************************/
	STDMETHOD(DeviceStateChanged)(struct ICOMStateDeviceEventArgs *args)	
	{
		DeviceChangeState deviceState;
		args->get_DeviceState(&deviceState);
	...
	STDMETHOD(Suspending)(struct _EventArgs *args)						
	{
		string outstr;
		ostringstream tmpstrm;
	...
	STDMETHOD(Resuming)(struct _EventArgs *args)						
	{
		string outstr;
		ostringstream tmpstrm;
	...
	STDMETHOD(QueryConfigChange)(struct _EventArgs *args)				
	{
		string outstr;
		ostringstream tmpstrm;
...
public:	
	 ...
	 * Author:		Andrija M. 02/13/2012
	 ****************************************************************/
	STDMETHOD(CallRequestEventHandler)(struct ICOMCallRequestEventArgs *args)	
	{
		CComPtr<ICOMContact> contact;
...
public:	
	...
	STDMETHOD(TalkPressed)(struct _DeviceEventArgs *args)			
	{
		string outstr;
		ostringstream tmpstrm;
	...
	STDMETHOD(ButtonPressed)(struct ICOMDeviceEventArgs *args)			
	{
	...
	STDMETHOD(MuteStateChanged)(struct ICOMDeviceEventArgs *args)		
	{
	...
	STDMETHOD(AudioStateChanged)(struct ICOMDeviceEventArgs *args)		
	{
		AudioType audioType;
...
public:	
	...
	STDMETHOD(HeadsetButtonPressed)(struct ICOMDeviceListenerEventArgs *args)			
	{
        KeepDialToneSuppressed(true);

		DeviceHeadsetButton button;
		args->get_HeadsetButton(&button);
		string outstr;
		ostringstream tmpstrm;
	...
	STDMETHOD(HeadsetStateChanged)(struct ICOMDeviceListenerEventArgs *args)			
	{
	...
	STDMETHOD(BaseButtonPressed)(struct ICOMDeviceListenerEventArgs *args)		
	{
		DeviceBaseButton button;
		COMDeviceEventType evType;
		SHORT dialedKey;
		args->get_BaseButton(&button);
		args->get_DeviceEventType(&evType);
	...
	STDMETHOD(BaseStateChanged)(struct ICOMDeviceListenerEventArgs *args)		
	{
		DeviceBaseStateChange basestateChanged;
    ...
    STDMETHOD(ATDStateChanged)(struct ICOMDeviceListenerEventArgs *args)								
	{
...
public:	
	...
	STDMETHOD(HeadsetStateChanged)(struct ICOMHeadsetStateEventArgs *args)			
	{
		string outstr;
		ostringstream tmpstrm;
		...
		HRESULT hRes = args->get_SerialNumber(&serNum);
		//if (!SUCCEEDED(hRes))
...
public:	
	...
	//IID_IDeviceCOMEventsExt Methods
	STDMETHOD(BaseEventReceived)(struct BaseEventArgs *args)			
	{
		string outstr;
		ostringstream tmpstrm;
...
CComPtr<ICOMSessionManager> g_pSessMgr;
CComPtr<ICOMSession> g_pSession;
ICOMHostCommandPtr g_pHostCommand;
ICOMHostCommandExtPtr g_pHostCommandExt;
ICOMATDCommandPtr g_pAtdCommand;
CComPtr<ICOMDeviceListener> g_pDeviceListener;
CComPtr<ICOMDeviceEvents> g_pDeviceEvents;
ICOMDevicePtr g_pActiveDevice = NULL;
CComPtr<ICOMCallCommand> g_pCallCommand;
CComPtr<ICOMCallEvents> g_pCallEvents;
...
void PrintSession()
{
	...
	g_pSession->get_SessionId(&sessionID);
...
void RegisterForProximity()
{
    ...
    if (g_pHostCommandExt != NULL)
    {
		...
		if (bSuccess != FALSE)
		{
			g_pHostCommandExt->RequestProximity(&bSuccess);
			if (bSuccess != FALSE)
			{
				string outstr;
				ostringstream tmpstrm;
...
void AttachDevice()
{
	...
	if( g_pActiveDevice != NULL)
	{
		...
		if( SUCCEEDED(g_pActiveDevice->get>get_DeviceEvents(&g_pDeviceEvents)))
		{
