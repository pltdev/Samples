...
BEGIN_MESSAGE_MAP(CWrapperSampleAppDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_HOLDCALL, &CWrapperSampleAppDlg::OnBnClickedHoldcall)
	ON_BN_CLICKED(IDC_RESUMECALL, &CWrapperSampleAppDlg::OnBnClickedResumecall)
END_MESSAGE_MAP()
...
void CWrapperSampleAppDlg::UpdateUI()
{
	...
	if (_attached) // && _infoCount > 0)
	{
		...
		GetDlgItem(IDC_HOLDCALL)->EnableWindow(_oncall); //countActiveCalls() > 0);
		GetDlgItem(IDC_RESUMECALL)->EnableWindow(_oncall); //countActiveCalls() > 0);
	}
	else
	{
		...
		GetDlgItem(IDC_HOLDCALL)->EnableWindow(FALSE);
		GetDlgItem(IDC_RESUMECALL)->EnableWindow(FALSE);
	}
...
void CWrapperSampleAppDlg::Spokes_MultiLineStateChanged(EventArgs * e)
{
	CString buffer;
	buffer.Format(">>> Multi line state has changed: \r\n%s%s\r\n%s%s\r\n%s%s\r\n%s%s\r\n%s%s\r\n%s%s",
		"PC Active: ", m_spokes->m_ActiveHeldFlags.m_bPCActive ? "Yes" : "No",
		"Mobile Active: ", m_spokes->m_ActiveHeldFlags.m_bMobileActive ? "Yes" : "No",
		"Deskphone Active: ", m_spokes->m_ActiveHeldFlags.m_bDeskphoneActive ? "Yes" : "No",
		"PC Held: ", m_spokes->m_ActiveHeldFlags.m_bPCHeld ? "Yes" : "No",
		"Mobile Held: ", m_spokes->m_ActiveHeldFlags.m_bMobileHeld ? "Yes" : "No",
		"Deskphone Held: ", m_spokes->m_ActiveHeldFlags.m_bDeskphoneHeld ? "Yes" : "No"
	);
	DebugPrint(__FUNCTION__, buffer.GetBuffer());
...
void CWrapperSampleAppDlg::Spokes_Connected(EventArgs * e)
{
	CString buffer;
	buffer.Format(">>> Headset was Connected to QD connector");
	DebugPrint(__FUNCTION__, buffer.GetBuffer());
}

void CWrapperSampleAppDlg::Spokes_Disconnected(EventArgs * e)
{
	CString buffer;
	buffer.Format(">>> Headset was Disconnected from QD connector");
	DebugPrint(__FUNCTION__, buffer.GetBuffer());
}
...
void CWrapperSampleAppDlg::OnBnClickedHoldcall()
{
...
void CWrapperSampleAppDlg::OnBnClickedResumecall()
{
	// TODO: Add your control notification handler code here
