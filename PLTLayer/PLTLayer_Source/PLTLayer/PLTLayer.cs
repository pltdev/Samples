 ...
 * Changed by: Lewis Collins
 *   Changes:
 *     - Adding function setassoftphone allowing apps to set themselves to
 *       receive call requested events from dialpad devices e.g. Calisto P240.
 *       Note: the softphone is automatically restored when PLTLayer is disposed.
 *     
 * Version 1.0.0.3:
 * Date: 10th Dec 2015
 * Compatible with Plantronics Hub version(s): 3.7.x
    ...
    public sealed class PLTLayer : DebugLogger
    {
        ...
        public void setup(string AppName = "PLTLayer App")
        {
            try
            {
                ...
                m_spokes.LineActiveChanged += new Spokes.LineActiveChangedEventHandler(m_spokes_LineActiveChanged);
        ...
        /// </summary>
        public void setassoftphone()
        {
            if (m_spokes != null)
            {
                m_previousSoftphone = m_spokes.GetDefaultSoftphone();
                m_spokes.SetDefaultSoftphone();
            }
            else
            {                
                DebugPrint(MethodInfo.GetCurrentMethod().Name, "WARNING: Attempt made to set softphone before call to PLTLayer's setup function was called.");
            }
        }

        private void restoresoftphone()
        {
            if (m_spokes != null)
            {
            ...
            {
                DebugPrint(MethodInfo.GetCurrentMethod().Name, "INFO: Attempt made to set softphone but m_spokes was null.");
            }
        }
    }
